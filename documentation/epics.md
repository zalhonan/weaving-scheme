...
### Функционал
- Масштабирование колесом мыши (0.5–50 px/клетка, мультипликативный шаг,
  относительно курсора)
- Адаптивный рендер при сильном zoom-out: номера/хвостики/мелкая сетка
  скрываются автоматически; viewport culling для канв 1000×1000
- Кнопка / хоткей «Вписать канву в экран» (`0`)
...
- Загрузка из JSON
- Печать в PDF:
  - Лист А4, ориентация портретная / альбомная
  - Произвольное число клеток на страницу (без верхней границы)
  - Кнопка «Уместить на 1 страницу» — авторасчёт клеток так, чтобы
    вся канва уместилась на 1 листе (1000×1000 → 1 страница)
  - Адаптивный рендер при сильном сжатии: номера / минорная / мажорная
    сетка скрываются по порогам, толщина линий ужимается до клетки
