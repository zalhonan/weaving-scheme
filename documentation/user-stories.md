...
**Критерии приёмки:**
- [ ] Кнопка "Печать PDF" в меню
- [ ] Формат: А4
- [ ] Поле «Клеток на страницу» определяет размер клетки
      (`cellSizeMM = USABLE_WIDTH / cellsPerPageX`)
- [ ] Если схема большая — автоматическая разбивка на несколько страниц
- [ ] Страницы пронумерованы для удобства склейки (`Page N / M
      [column X, row Y]`)
- [ ] Генерируется PDF файл

---

### US-5.4: Произвольный масштаб печати, ориентация и «Уместить на 1 страницу»
**Как** пользователь
**Я хочу** иметь полный контроль над масштабом печати
**Чтобы** напечатать любую схему — от детальной (большие клетки) до
обзорной миниатюры (1000×1000 на одной странице)

**Критерии приёмки:**
- [ ] Поле «Клеток на страницу» принимает любое число ≥ 1 (без верхней
      границы — сняли прежний кап в 100)
- [ ] Селектор «Ориентация» (Портретная / Альбомная) меняет ориентацию
      A4 в выходном PDF и пересчитывает доступную область печати
- [ ] Кнопка «Уместить на 1 страницу» рядом с полем — выставляет
      `cellsPerPageX = max(width, ⌈height × USABLE_W / USABLE_H⌉)` для
      текущей ориентации; результат гарантированно укладывается на
      1 страницу
- [ ] При `cellSizeMM < 1.8` номера колонок/строк скрываются автоматически
- [ ] При `cellSizeMM < 0.5` минорная сетка скрывается
- [ ] При `cellSizeMM < 0.3` мажорная сетка скрывается
- [ ] При `cellSizeMM < 0.4 / 0.9` толщина пользовательских линий
      ужимается до `cellSizeMM × 0.9` — линии не размазываются на
      соседние клетки
- [ ] При `cellsPerPage` 1..100 в портретной ориентации выход PDF
      идентичен прежнему (регрессия)
- [ ] Ориентация и значение поля сбрасываются при перезагрузке страницы
      (session-only)
...
| Экспорт JSON | US-5.1 |
| Импорт JSON | US-5.2 |
| Печать PDF А4 | US-5.3 |
| Разбивка на страницы + нумерация | US-5.3 |
| Ориентация портретная / альбомная | US-5.4 |
| Произвольное число клеток на страницу | US-5.4 |
| Кнопка «Уместить на 1 страницу» | US-5.4 |
| Адаптивный рендер PDF (миниатюра 1000×1000) | US-5.4 |
